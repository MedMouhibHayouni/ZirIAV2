...
@Component({
  ...
  styles: [`
    ...
    .inst-btn--primary { background: var(--zir-emerald); color: #fff; }
    ...
    .inst-field select:focus { border-color: var(--zir-emerald); }
    ...
    .role-badge { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 600; }
    .role-badge--director { background: rgba(245,158,11,0.15); color: #f59e0b; }
    .role-badge--agent { background: rgba(16,185,129,0.15); color: #10b981; }
    .role-badge--viewer { background: rgba(99,102,241,0.15); color: #6366f1; }
    ...
    .sk--icon { width: 40px; height: 40px; border-radius: 12px; }
    ...
    .member-count { font-size: 2rem; font-weight: 700; color: var(--zir-emerald); }
    ...
    .toast--success { background: var(--zir-emerald); color: #fff; }
      ...
      <div class="inst-header">
        <div class="inst-header__left">
          <div class="inst-header__icon">
            <ng-icon name="lucideBuilding2" size="20" style="color: var(--zir-emerald)"></ng-icon>
          </div>
          <div>
            <h1 class="inst-header__title">Mon Bureau & Équipe Régionale</h1>
            <p class="inst-header__sub">Coordonnées publiques de l'antenne et gestion des agents du bureau</p>
          </div>
        </div>
        @if (isDirector()) {
          ...
          @if (loading()) {
            ...
            <div class="office-info-grid">
              <div class="office-info-item">
                <p>Dénomination</p>
                <p>{{ officeName() || 'Bureau APIA Régional' }}</p>
              </div>
              <div class="office-info-item">
                <p>Gouvernorat</p>
                <p>{{ governorate() || '—' }}</p>
              ...
              <div class="office-info-item">
                <p>Boundary ID</p>
                <p style="font-family: monospace; font-size: 0.78rem;">GOV-TN-{{ governorate()?.substring(0,3)?.toUpperCase() || 'UNK' }}</p>
              </div>
            </div>
          }
        </div>
            ...
            <table class="members-table">
              ...
              <tbody>
                @for (member of members(); track member.id) {
                  <tr>
                    <td>
                      <div class="member-name">{{ member.user?.name || 'Agent APIA' }}</div>
                    </td>
                    <td>
                    ...
                    <td>
                      @if (isDirector() && !isSelf(member)) {
                        <select
                          style="padding: 4px 8px; border-radius: 8px; font-size: 0.75rem; background: var(--bg-secondary); border: 1px solid var(--border); color: var(--text-primary); cursor: pointer;"
                          [ngModel]="member.officeRole"
                        ...
                        <span class="role-badge" [ngClass]="getRoleBadgeClass(member.officeRole)">
                          ...
                          {{ getRoleLabel(member.officeRole) }}
                        </span>
                      }
                    </td>
                    <td>
                      <span class="status-pill" [style.color]="member.isActive ? 'var(--zir-emerald)' : 'var(--text-muted)'">
        ...
        <div class="inst-modal-backdrop" (click)="closeModal()">
          <div class="inst-modal" (click)="$event.stopPropagation()">
            ...
            <div class="inst-field">
              ...
              <input id="member-email" type="email" [(ngModel)]="form().email" placeholder="email@institution.tn" />
            </div>
...
export class ApiaMonBureauComponent implements OnInit {
